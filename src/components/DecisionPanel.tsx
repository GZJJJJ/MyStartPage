    ...
    <Panel title="决定器" subtitle={`${parsedOptions.length} 个选项`} icon={Shuffle} className={className}>
      <div className="grid gap-4 md:grid-cols-[1fr_13rem]">
        <textarea
          ...
          placeholder={"每行一个选项\n例如：写作业\n看文档\n休息十分钟"}
          aria-label="随机决定选项"
          className={`${inputClass} min-h-36 text-sm leading-6`}
        />
        <div className="flex flex-col gap-4 rounded-[24px] border border-white/32 bg-white/18 p-4 dark:border-white/10 dark:bg-slate-950/18">
          <button type="button" onClick={decide} className={primaryButtonClass}>
            <Shuffle size={17} />
            抽一个
          </button>
          <div className="flex min-h-24 items-center justify-center rounded-[20px] bg-white/24 px-4 text-center text-base font-medium text-[#344046] dark:bg-slate-950/24 dark:text-slate-100">
            {result || "等待结果"}
          </div>
        </div>
      </div>
